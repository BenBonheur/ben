...
const StockOut: React.FC = () => {
  ...
  const fetchStockOutHistory = async () => {
    try {
      ...
      toast.error('Error fetching stock out history');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const selectedPart = spareParts.find(part => part.id === formData.sparePartId);
      if (!selectedPart) {
        toast.error('Please select a spare part');
        return;
      }

      if (formData.stockOutQuantity > selectedPart.quantity) {
      ...
      toast.error('Error recording stock out');
    }
  };
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          stockOutQuantity: parseInt(e.target.value),
                        })
                      }
